...
class PoseLandmarkerHelper(
    ...
    /**
     * 카메라에서 이미지를 캡쳐하여 해당 이미지를 비트맵으로 변환
     * -> 미디어파이프를 사용하여 이미지를 감지
     * */
    fun detectLiveStream(
        ...
        // Bitmap.createBitmap을 사용하여 이미지 프록시의 너비와 높이와 같은 크기의 비트맵 버퍼 생성
        val bitmapBuffer =
            Bitmap.createBitmap(
                imageProxy.width,
                imageProxy.height,
        ...
        // imageProxy닫기
        // imageProxy.use : 블록 내에서 리소스를 사용한 후 자동으로 리소스 해제
        // imageProxy를 가져와 블록내 사용 후 자동으로 ImageProxy를 닫아 리소스 관리함
//        imageProxy.use { bitmapBuffer.copyPixelsFromBuffer(imageProxy.planes[0].buffer) }
//        imageProxy.close()
        bitmapBuffer.copyPixelsFromBuffer(imageProxy.planes[0].buffer)

        // 회전 및 뒤집기 작업 수행하는 Matrix객체
        val matrix = Matrix().apply {
            // Rotate the frame received from the camera to be in the same direction as it'll be shown
            postRotate(imageProxy.imageInfo.rotationDegrees.toFloat())

            // 이미지의 회전 각도를 이미지 프록시에서 가져와 이미지를 원하는 방향으로 회전
            // 전면 카메라 사용시 이미지를 수평으로 뒤집기
            if (isFrontCamera) {
                postScale(
                    -1f,
                    1f,
        ...
        // rotatedBitmap을 MPImage객체로 변환 -> 미디어파이프에서 이미지를 처리할 수 있음
        val mpImage = BitmapImageBuilder(rotatedBitmap).build()

        // MPImage를 사용하여 이미지 감지를 비동기적으로 시작
        // frameTime은 현재 프레임의 타임스탬프로 전달 -> 프레임간의 시간 차이를 추적하는데 사용
